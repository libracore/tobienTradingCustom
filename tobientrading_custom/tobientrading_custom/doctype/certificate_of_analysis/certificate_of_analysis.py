...
def create_coa_from_xml_data(data):
    ...
    coa_name = soup.find("sample_sample_name").text
    
    # Create COA if not exists
    coa = frappe.db.exists("Certificate of Analysis", {"certificate_of_analysis": coa_name})
    if coa:
        coa = frappe.get_doc("Certificate of Analysis", coa)
    else:
        coa = frappe.new_doc("Certificate of Analysis")

        end_of_analysis = datetime.strptime(soup.find("Reportdate").text, '%Y-%m-%d').strftime('%Y-%m-%d')
        item_code = soup.find("Referencenumber").text

        item = frappe.get_doc("Item", item_code)
        batch = soup.find("BatchLotnumber").text
        ...
        coa.update({
            "certificate_of_analysis": coa_name,
            "item": item_code,
            "item_name": item.item_name,
            "item_group": item.item_group,
            "date_coa": end_of_analysis,
            "begin_of_analysis": end_of_analysis,
            "end_of_analysis": end_of_analysis,
